...
public class PasswordChangeEventPublisher implements ApplicationEventPublisherAware {
    ...
    public void passwordFailure(String userId, Exception e) {
        ...
        publish(new PasswordChangeFailureEvent(e.getMessage(), user, getPrincipal(), IdentityZoneHolder.get()));
    }

    public void passwordChange(String userId) {
        publish(new PasswordChangeEvent("Password changed", getUser(userId), getPrincipal(), IdentityZoneHolder.get()));
    }
