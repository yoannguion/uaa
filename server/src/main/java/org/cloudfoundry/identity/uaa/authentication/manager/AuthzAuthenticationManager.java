...
public class AuthzAuthenticationManager implements AuthenticationManager, ApplicationEventPublisherAware {
    ...
    public Authentication authenticate(Authentication req) throws AuthenticationException {
        ...
        if (user == null) {
            ...
            publish(new UserNotFoundEvent(req, IdentityZoneHolder.get()));
        } else {
            ...
            if (!passwordMatches) {
                ...
                publish(new IdentityProviderAuthenticationFailureEvent(req, req.getName(), OriginKeys.UAA, IdentityZoneHolder.get()));
                publish(new UserAuthenticationFailureEvent(user, req, IdentityZoneHolder.get()));
            } else {
                ...
                if (!(allowUnverifiedUsers && user.isLegacyVerificationBehavior()) && !user.isVerified()) {
                    publish(new UnverifiedUserAuthenticationEvent(user, req, IdentityZoneHolder.get()));
                ...
                publish(new IdentityProviderAuthenticationSuccessEvent(user, success, OriginKeys.UAA, IdentityZoneHolder.get()));
                return success;
            }
        }
